...
export const load: PageServerLoad = async ({ url }) => {
  ...
  const leaguePlayers: (typeof save.players[string] & { teamId: string, teamName: string })[] = [];
  ...
  for (const teamId of activeLeague.teams) {
      ...
      if (team) {
          for (const playerId of team.players) {
              ...
              if (player) {
                  leaguePlayers.push({ ...player, teamId: team.id, teamName: team.name });
              }
          }
      }
  }
  ...
  const topCleanSheets = [...leaguePlayers]
      .filter(p => p.role === 'GK' && p.seasonStats && p.seasonStats.cleanSheets > 0)
