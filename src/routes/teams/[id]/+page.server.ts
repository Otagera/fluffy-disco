...
import { addInboxMessage, loadSaveGame, writeSaveGame } from "$lib/data/store";
...
export const actions: Actions = {
	...
	submitTransferBid: async ({ request, params }) => {
		...
		if (!player || !sellingTeam)
			return fail(404, { message: "Player or team not found" });
	...
	assignScout: async ({ request }) => {
		...
		const existing = save.scoutingReports.find(
