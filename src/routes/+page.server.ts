...
export const load: PageServerLoad = async () => {
	...
	const nextFixture = save.fixtures.find(
		(f) =>
			!f.played &&
			(f.homeTeamId === managerTeamId || f.awayTeamId === managerTeamId),
	);

	let opponentTeam = null;
	if (nextFixture) {
		const oppId =
			nextFixture.homeTeamId === managerTeamId
...
export const actions: Actions = {
	advanceDay: async ({ request }) => {
		const formData = await request.formData();
		const delegate = formData.get("delegate") === "true";

		const save = loadSaveGame();
		if (!save) return fail(404, { error: "Save not found" });

		const result = advanceOneDay(save, { delegate });
		return { success: true, ...result, currentDate: save.currentDate };
	},
	sendSkipSummary: async ({ request }) => {
		const formData = await request.formData();
		const actions = formData.get("actions")?.toString().split(",") || [];

		const save = loadSaveGame();
		if (!save) return fail(404, { error: "Save not found" });

		sendSkipSummary(save, actions);
		return { success: true };
	},
