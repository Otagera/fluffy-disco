...
export const load: PageServerLoad = async ({ url }) => {
	...
	const allPlayers = Object.values(save.players);
	...
	const filteredPlayers = allPlayers.filter((p) => {
		...
		if (knowledgeFilter !== "") {
			const report = (save.scoutingReports || []).find(
				(r) => r.playerId === p.id && r.teamId === save.manager.teamId,
			);
	...
	filteredPlayers.sort((a, b) => {
		const reportA = (save.scoutingReports || []).find(
			(r) => r.playerId === a.id && r.teamId === save.manager.teamId,
		);
		const reportB = (save.scoutingReports || []).find(
			(r) => r.playerId === b.id && r.teamId === save.manager.teamId,
		);
	...
	const shortlistedPlayers = (save.shortlist || [])
