<script lang="ts">
  ...
  // Camera Logic
  let cameraTransform = $derived.by(() => {
    const cam = matchState.camera;
    // Simpler approach: translate the whole coordinate space relative to the 0.5, 0.5 center
    const offsetX = (0.5 - cam.x) * PITCH_W * cam.zoom;
    const offsetY = (0.5 - cam.y) * PITCH_H * cam.zoom;
    
    return `translate(${offsetX}px, ${offsetY}px) scale(${cam.zoom})`;
  });
...
<div class="pitch-container">
  ...
  <svg viewBox="-10 -5 {PITCH_W + 20} {PITCH_H + 10}" class="pitch-svg" preserveAspectRatio="xMidYMid meet">
    <defs>
      ...
      <radialGradient id="pitch-light" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
    ...
    <!-- 0. Run-off area -->
    <rect x="-10" y="-5" width="{PITCH_W + 20}" height="{PITCH_H + 10}" fill="#111" />

    <g style="transform: {cameraTransform}; transform-origin: center; will-change: transform;">
      <!-- 1. Base green rect -->
      ...
      <rect width="{PITCH_W}" height="{PITCH_H}" fill="url(#pitch-light)" pointer-events="none" />
