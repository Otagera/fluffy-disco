export type TacticalStyle = 'Tiki-Taka' | 'Gegenpress' | 'Fluid Counter' | 'Route One' | 'Park the Bus' | 'Balanced';
export type Mentality = 'ULTRA_ATTACKING' | 'ATTACKING' | 'BALANCED' | 'DEFENSIVE' | 'ULTRA_DEFENSIVE';
...
export const TACTICAL_COMPATIBILITY: Record<TacticalStyle, CompatibilityMatrix> = {
    'Tiki-Taka': {
        style: 'Tiki-Taka',
        preferredMentalities: ['ATTACKING', 'BALANCED'],
        formationKeywords: ['4-3-3', '4-2-3-1', 'Diamond'],
        description: 'Focuses on short passing and high possession. Best with technical players and expansive formations.'
    },
    'Gegenpress': {
        style: 'Gegenpress',
        preferredMentalities: ['ULTRA_ATTACKING', 'ATTACKING'],
        formationKeywords: ['4-3-3', '4-4-2 Wide', '4-2-3-1'],
        description: 'High intensity pressing to win the ball back immediately. Requires high stamina and aggressive mindsets.'
    },
    'Fluid Counter': {
        style: 'Fluid Counter',
        preferredMentalities: ['BALANCED', 'DEFENSIVE'],
        formationKeywords: ['3-5-2', '5-3-2', '4-4-1-1', '4-1-4-1'],
        description: 'Absorb pressure and break with speed. Best with a solid defensive base and fast attackers.'
    },
    'Route One': {
        style: 'Route One',
        preferredMentalities: ['BALANCED', 'DEFENSIVE'],
        formationKeywords: ['4-4-2', '5-4-1', '3-4-3'],
        description: 'Long balls to a target man. Simple, direct, and effective with physically strong players.'
    },
    'Park the Bus': {
        style: 'Park the Bus',
        preferredMentalities: ['DEFENSIVE', 'ULTRA_DEFENSIVE'],
        formationKeywords: ['5-4-1', '4-5-1', '5-3-2'],
        description: 'Extreme defensive focus. Aiming to concede zero space and frustrate the opponent.'
    },
    'Balanced': {
        style: 'Balanced',
 ...
 */
export function getTacticalCompatibility(style: string, mentality: string, formation: string): number {
    ...
    if (config.formationKeywords.length > 0) {
        const matchesFormation = config.formationKeywords.some(k => formation.includes(k));
        if (matchesFormation) {
            score += 0.25;
        }
