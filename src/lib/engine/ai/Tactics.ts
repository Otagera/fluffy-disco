...
export class TacticalManager {
	...
	calculateAnchors(
		...
		const homePressers = new Set(
			homeDistances.slice(0, homePressCount).map((d) => d.idx),
		);
		const awayPressers = new Set(
			awayDistances.slice(0, awayPressCount).map((d) => d.idx),
		);
		...
		for (let i = 0; i < PLAYER_COUNT; i++) {
			...
			let isPressing =
				(team === 0 && homePressers.has(i)) ||
				(team === 1 && awayPressers.has(i));

			if (role === "BWM" && !isPossession && !isPressing) {
				const px = playerBuffer
					? playerBuffer[i * PLAYER_STRIDE + PLAYER_OFFSET_X]
					: base.x;
				const py = playerBuffer
					? playerBuffer[i * PLAYER_STRIDE + PLAYER_OFFSET_Y]
					: base.y;
			...
			if (isPressing && (!isPossession || isBallLoose)) {
				...
				const lookaheadTime = Math.min(
					distToBall / (8.0 * pursuitAggression),
					1.5,
				);
				...
				const activeOffsideLine =
					team === 0 ? offsideLineTeam1 : offsideLineTeam0;
				...
				if (["W", "IF", "ST", "AF"].includes(role)) {
					...
					if (inOpponentHalf) {
						...
						const runCommitment =
							(positioning / 100) * (0.5 + anticipation / 200);
