...
export function calculatePlayerOverall(
	player: Pick<
		PlayerProfile,
		"role" | "attributes" | "condition" | "potential" | "birthDate" | "morale"
	>,
	currentDate: string,
	...
	const developmentGap =
		(player.potential - base) * (currentAge < 24 ? 0.15 : 0.05);
	...
	if (morale < 30)
		score -= (30 - morale) / 10; // Max -3 penalty
...
export function calculateTeamOverall(
	...
	const squad = team.players
		...
		.sort(
			(a, b) =>
				(b.overall ?? calculatePlayerOverall(b, currentDate)) -
				(a.overall ?? calculatePlayerOverall(a, currentDate)),
		);

	const selected: PlayerProfile[] = [];

	for (const [role, target] of Object.entries(TEAM_ROLE_TARGETS) as [
		Role,
		number,
	][]) {
	...
	const total = selected.reduce(
		(sum, player) =>
			sum + (player.overall ?? calculatePlayerOverall(player, currentDate)),
		0,
	);
...
export function calculatePlayerValue(
	...
	const baseValue = 1.45 ** ovr * 250000;
	...
	if (currentAge < 21) ageMod = 1.4;
		...
		ageMod = Math.max(0.05, 1.0 - (currentAge - 29) * 0.12);
