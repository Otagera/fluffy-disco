...
function randomName(style: string = "Global") {
	...
	if (isGlobal || rand > 0.8) {
		if (rand < (isGlobal ? 0.1 : 0.05))
			return `${anglicizedNames.chinese.first[getRandomInt(0, anglicizedNames.chinese.first.length - 1)]} ${anglicizedNames.chinese.last[getRandomInt(0, anglicizedNames.chinese.last.length - 1)]}`;
		if (rand < (isGlobal ? 0.2 : 0.1))
			return `${anglicizedNames.japanese.first[getRandomInt(0, anglicizedNames.japanese.first.length - 1)]} ${anglicizedNames.japanese.last[getRandomInt(0, anglicizedNames.japanese.last.length - 1)]}`;
		if (rand < (isGlobal ? 0.3 : 0.15))
			return `${anglicizedNames.arabic.first[getRandomInt(0, anglicizedNames.arabic.first.length - 1)]} ${anglicizedNames.arabic.last[getRandomInt(0, anglicizedNames.arabic.last.length - 1)]}`;
		if (rand < (isGlobal ? 0.4 : 0.2))
			return `${anglicizedNames.nigerian.first[getRandomInt(0, anglicizedNames.nigerian.first.length - 1)]} ${anglicizedNames.nigerian.last[getRandomInt(0, anglicizedNames.nigerian.last.length - 1)]}`;
...
function generatePlayer(
	...
	const attr = (key: keyof PlayerProfile["attributes"], spread = 2.2) => {
		const value =
			baseAbility + roleBoost(role, key) + randomFloat(-spread, spread);
	...
	return {
		...
		wage: Math.round(
			baseAbility * 1000 * (level === 1 ? 5 : level === 2 ? 2 : 1),
		),
...
function pickStartingXI(players: PlayerProfile[]) {
	...
	return [
		...
		...byRole.MID.slice(0, 3),
		...byRole.FWD.slice(0, 3),
	];
}
...
export function generateTeam(
	...
	style: string = "Global",
): { team: TeamProfile; players: PlayerProfile[] } {
	let name = "";
	...
	do {
		...
		if (
			["Spanish", "Italian", "French", "German"].includes(style) &&
			Math.random() > 0.5
		) {
	...
	const teamAbility = clamp(
		profile.abilityBase +
			randomFloat(-profile.abilityVariance, profile.abilityVariance),
		4,
		19,
	);
	...
	const mentality =
		config.preferredMentalities[
			getRandomInt(0, config.preferredMentalities.length - 1)
		];

	const formationNames = Object.keys(formations);
	const compatibleFormations = formationNames.filter((f) =>
		config.formationKeywords.some((k) => f.includes(k)),
	);
	const formation =
		compatibleFormations.length > 0 && Math.random() > 0.3
	...
	const rawPlayers = rolePool.map((role) => {
		const p = generatePlayer(
			role,
			teamAbility + randomFloat(-1.2, 1.2),
			level,
			profile.youthWeight,
			style,
		);
	...
	team.overall = calculateTeamOverall(
		team,
...
export function generateSaveGame(
	managerName: string,
	style: string = "Global",
	...
	levels.forEach((lvl, i) => {
		...
		saveGame.fixtures.push(
			...generateFixtures(league.teams, league.id, saveGame.currentDate),
		);
...
if (
	...
	(typeof process !== "undefined" &&
		process.argv[1] === path.resolve("src/lib/data/generator.ts"))
) {
	const { saveNewGameToDB } = require("./store.ts");
	const save = generateSaveGame("The Gaffer");
	saveNewGameToDB(save);
	console.log(
		`Generated and saved game state to SQLite DB for ${Object.keys(save.teams).length} teams.`,
	);
}
