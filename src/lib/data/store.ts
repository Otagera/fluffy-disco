...
export function saveNewGameToDB(save: SaveGame) {
	db.transaction((tx) => {
		...
		for (const p of Object.values(save.players)) {
			const team = Object.values(save.teams).find((t) =>
				t.players.includes(p.id),
			);
...
export function loadSaveGame(): SaveGame | null {
	try {
		...
		for (const l of leagues) {
			const news = db
				.select()
				.from(schema.leagueNews)
				.where(eq(schema.leagueNews.leagueId, l.id))
				.all();
			const leagueStandings = db
				.select()
				.from(schema.standings)
				.where(eq(schema.standings.leagueId, l.id))
				.all();
			const leagueTeams = db
				.select()
				.from(schema.teams)
		...
		for (const f of fixtures) {
			const goalEvents = db
				.select()
				.from(schema.fixtureGoals)
				.where(eq(schema.fixtureGoals.fixtureId, f.id))
				.all();
			save.fixtures.push({
				...f,
				goalEvents: goalEvents.map((g) => ({
					playerId: g.playerId,
					minute: g.minute,
					teamId: g.teamId,
				})),
	...
	try {
		db.transaction((tx) => {
			...
			for (const l of saveData.leagues) {
				if (l.news) {
					tx.delete(schema.leagueNews)
						.where(eq(schema.leagueNews.leagueId, l.id))
						.run();
					for (const n of l.news) {
						tx.insert(schema.leagueNews)
							.values({
								id: n.id,
								leagueId: l.id,
								week: n.week,
...
export function sendSkipSummary(save: SaveGame, actions: string[]) {
	if (actions.length === 0) return;

	const summaryBody = `During the last skip period, I handled the following on your behalf:\n\n${actions.map((a) => `• ${a}`).join("\n")}\n\nEverything is up to date and ready for your review.`;

	addInboxMessage(save, {
		teamId: save.manager.teamId,
		date: save.currentDate,
		sender: "Assistant Manager",
		subject: "Delegation Summary",
		body: summaryBody,
		type: "LEAGUE",
		isUrgent: false,
	});
...
function recalculateOverallRatings(save: SaveGame) {
	for (const player of Object.values(save.players))
		player.overall = calculatePlayerOverall(player, save.currentDate);
	for (const team of Object.values(save.teams))
		team.overall = calculateTeamOverall(team, save.players, save.currentDate);
}

export function addInboxMessage(
	save: SaveGame,
	message: Omit<InboxMessage, "id" | "isRead">,
) {
...
function processAITransfers(save: SaveGame, skipConfig?: SkipConfig): boolean {
	if (!isTransferWindowOpen(save.currentDate)) return false;

	let hadUrgentOffer = false;

	// To prevent the game from freezing, only evaluate ~2 random CPU teams per day
	const allTeams = Object.values(save.teams).filter(
		(t) => t.id !== save.manager.teamId,
	);
	...
	for (const team of teamsToEvaluate) {
		...
		if (targetRole) {
			...
			const potentialTargets = Object.values(save.players).filter(
				(p) =>
					p.role === targetRole &&
					p.teamId !== team.id &&
				...
				if (val < budget) {
					...
					if (target.teamId === save.manager.teamId) {
						...
						if (skipConfig?.delegate) {
							// Assistant automatically rejects for now
							addInboxMessage(save, {
								teamId: save.manager.teamId,
								date: save.currentDate,
								sender: "Assistant Manager",
								subject: `Delegated: Rejected Offer for ${target.name}`,
								body: `I have automatically rejected a formal transfer fee of ${new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 }).format(bidAmount)} from ${team.name} for ${target.name}. We are not looking to sell at this price.`,
								type: "TRANSFER",
								isUrgent: false,
						...
						if (sellingTeam) {
							...
							if (Math.random() > 0.4) {
								...
								sellingTeam.transferBudget =
									(sellingTeam.transferBudget || 0) + bidAmount;

								sellingTeam.players = sellingTeam.players.filter(
									(id) => id !== target.id,
								);
								...
								// Add news
								const league = save.leagues.find((l) =>
									l.teams.includes(team.id),
								);
								if (league) {
...
export function advanceOneDay(
	...
	const hadUrgentOffer = processAITransfers(save, skipConfig);
	if (hadUrgentOffer) {
		if (skipConfig?.delegate) {
			delegatedActions.push(`Rejected a transfer offer from a CPU team.`);
		} else {
			mustStop = true;
					...
					if (oldTeamId === save.manager.teamId) {
						...
						if (skipConfig?.delegate) {
							delegatedActions.push(
								`Contract expired for ${player.name}. He is now a free agent.`,
							);
						} else {
							mustStop = true;
		...
		if (managerTeam && player.teamId === managerTeam.id) {
			...
			if (!isStarter && isStarPlayer) {
				...
				if (player.morale < 30) {
					...
					if (Math.random() < 0.1) {
						// Random chance to trigger inbox complain
						addInboxMessage(save, {
							teamId: managerTeam.id,
							date: save.currentDate,
							sender: player.name,
	...
	const todayFixture = save.fixtures.find(
		(f) =>
			f.played === false &&
			(f.homeTeamId === save.manager.teamId ||
				f.awayTeamId === save.manager.teamId) &&
			f.date === save.currentDate,
	);

	if (todayFixture) {
		if (skipConfig?.delegate) {
			processWeekResults(save, null);
			delegatedActions.push(
				`Simulated match: ${save.teams[todayFixture.homeTeamId].name} vs ${save.teams[todayFixture.awayTeamId].name}`,
			);
		} else {
			mustStop = true;
			reason = "Match Day";
...
export function processWeekResults(save: any, playerMatchResult: any) {
	try {
		...
		if (playerMatchResult) {
			const playerFixture = save.fixtures.find(
				(f: any) => f.id === playerMatchResult.fixtureId,
			);
			if (playerFixture) {
				...
				if (
					playerMatchResult.matchAnalytics &&
					playerMatchResult.matchAnalytics.events
				) {
						...
						if (event.type === "goal" && event.playerId !== undefined) {
							...
							if (scorer) {
								...
								for (let j = i - 1; j >= 0; j--) {
									...
									if (
										prevEvent.type === "pass" &&
										prevEvent.team === event.team &&
										prevEvent.playerId !== undefined &&
										prevEvent.playerId !== event.playerId
									) {
										const assister = fullSquad[prevEvent.playerId];
										if (assister) {
											if (!assister.seasonStats)
												assister.seasonStats = {
													apps: 0,
													goals: 0,
													assists: 0,
													cleanSheets: 0,
		...
		if (playerMatchResult?.playerStamina) {
			for (const [pId, stamina] of Object.entries(
				playerMatchResult.playerStamina,
			)) {
				if (save.players[pId]) {
					save.players[pId].condition = Math.max(1, stamina as number);
					if ((stamina as number) < 40 && Math.random() < 0.05)
						save.players[pId].injury = {
							type: "Muscle Strain",
							weeksRemaining: Math.floor(Math.random() * 3) + 1,
						};
				}
			}
		}
		teamsPlayed.forEach((tid) => {
			...
			if (team) {
				team.players.slice(0, 14).forEach((pId) => {
					if (
						playerMatchResult?.playerStamina &&
						playerMatchResult.playerStamina[pId] !== undefined
					)
						return;
					...
					if (p && !p.injury) {
						...
						if (p.condition < 40 && Math.random() < 0.02)
							p.injury = {
								type: "Knock",
								weeksRemaining: Math.floor(Math.random() * 2) + 1,
							};
					}
				});
			}
...
function simFixtures(
	...
	const getTeamConditionMod = (topPlayers: any[]) => {
		...
		const avgCondition =
			topPlayers.reduce(
				(sum: number, p: any) => sum + (p.condition ?? 100),
				0,
			) / topPlayers.length;
...
function generateNews(save: SaveGame, week: number) {
	for (const league of save.leagues) {
		...
		const weekFixtures = save.fixtures.filter(
			(f) => f.week === week && league.teams.includes(f.homeTeamId),
		);
			...
			if (margin >= 4) {
				const winner =
					f.homeScore! > f.awayScore!
						? save.teams[f.homeTeamId]
						: save.teams[f.awayTeamId];
				const loser =
					f.homeScore! > f.awayScore!
						? save.teams[f.awayTeamId]
						: save.teams[f.homeTeamId];
		...
		const allPlayersInLeague = Object.values(save.players).filter((p) =>
			league.teams.includes(p.teamId!),
		);
		const topScorer = allPlayersInLeague.sort(
			(a, b) => (b.seasonStats?.goals || 0) - (a.seasonStats?.goals || 0),
		)[0];
		...
		if (sorted.length >= 2) {
			...
			const clash = weekFixtures.find(
				(f) =>
					(f.homeTeamId === first && f.awayTeamId === second) ||
					(f.homeTeamId === second && f.awayTeamId === first),
			);
			if (clash) {
				league.news.push({
					id: `n_${Math.random().toString(36).slice(2, 11)}`,
...
function updateAllStandings(save: SaveGame) {
	for (const league of save.leagues) {
		...
		const leagueFixtures = save.fixtures.filter(
			(f: any) => f.played && f.leagueId === league.id,
		);
...
function advanceSeason(save: SaveGame) {
	...
	const resolvePlayoff = (teams: any[]) => {
	...
	const pro2 = [
		...s2.slice(0, 2).map((s) => s.teamId),
		resolvePlayoff(s2.slice(2, 6)),
	];
	const rel2 = s2.slice(-3).map((s) => s.teamId);
	const pro3 = [
		...s3.slice(0, 2).map((s) => s.teamId),
		resolvePlayoff(s3.slice(2, 6)),
	];
	const rel3 = s3.slice(-4).map((s) => s.teamId);
	const pro4 = [
		...s4.slice(0, 3).map((s) => s.teamId),
		resolvePlayoff(s4.slice(3, 7)),
	];
	const performSwap = (l: League, leave: string[], enter: string[]) => {
		l.teams = l.teams.filter((t) => !leave.includes(t));
	...
	save.leagues.forEach((league) => {
		...
		save.fixtures.push(
			...generateFixtures(league.teams, league.id, save.currentDate),
		);
	});
	return save;
}
...
export function toggleScoutingPriority(
	save: SaveGame,
	playerId: string,
): { success: boolean; message?: string } {
	if (!save.scoutingReports) save.scoutingReports = [];
	const report = save.scoutingReports.find(
		(r) => r.playerId === playerId && r.teamId === save.manager.teamId,
	);
	if (!report) return { success: false, message: "Scouting report not found" };

	if (!report.isPriority) {
		const priorityCount = save.scoutingReports.filter(
			(r) => r.isPriority && r.teamId === save.manager.teamId,
		).length;
		if (priorityCount >= 5) {
			return {
				success: false,
				message: "Already scouting 5 players (limit reached)",
			};
		}
